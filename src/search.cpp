    ...
    while ((move = mp.next_move()) != Move::none())
    {
            ...
            && ttData.depth >= depth - 3)
        {
            ...
            if (value < singularBeta)
            {
                ...
                // Query TTMoveHistory with multi-ply lookback (2, 4, 6, 8-ply)
                int ttmh = 0;
                ...
                {
                    Square prevTo = (ss-4)->currentMove.to_sq();
                    Piece prevPc = pos.piece_on(prevTo);
                    ttmh += 32 * ttMoveHistory[prevPc][prevTo][pos.moved_piece(move)][move.to_sq()] / 128;
                }
                if ((ss-6)->currentMove.is_ok())
                {
                    Square prevTo = (ss-6)->currentMove.to_sq();
                    Piece prevPc = pos.piece_on(prevTo);
                    ttmh += 16 * ttMoveHistory[prevPc][prevTo][pos.moved_piece(move)][move.to_sq()] / 128;
                }
                if ((ss-8)->currentMove.is_ok())
                {
                    Square prevTo = (ss-8)->currentMove.to_sq();
                    Piece prevPc = pos.piece_on(prevTo);
                    ttmh += 8 * ttMoveHistory[prevPc][prevTo][pos.moved_piece(move)][move.to_sq()] / 128;
                }
            ...
            {
                // Update TTMoveHistory with multi-ply context (2, 4, 6, 8-ply)
                ...
                if ((ss-4)->currentMove.is_ok())
                {
                    Square prevTo = (ss-4)->currentMove.to_sq();
                    Piece prevPc = pos.piece_on(prevTo);
                    ttMoveHistory[prevPc][prevTo][pos.moved_piece(move)][move.to_sq()] << (penalty * 32 / 137);
                }
                if ((ss-6)->currentMove.is_ok())
                {
                    Square prevTo = (ss-6)->currentMove.to_sq();
                    Piece prevPc = pos.piece_on(prevTo);
                    ttMoveHistory[prevPc][prevTo][pos.moved_piece(move)][move.to_sq()] << (penalty * 16 / 137);
                }
                if ((ss-8)->currentMove.is_ok())
                {
                    Square prevTo = (ss-8)->currentMove.to_sq();
                    Piece prevPc = pos.piece_on(prevTo);
                    ttMoveHistory[prevPc][prevTo][pos.moved_piece(move)][move.to_sq()] << (penalty * 8 / 137);
                }
                return value;
            }
    ...
    else if (bestMove)
    {
        ...
        if (!PvNode && ttData.move)
        {
            ...
            if ((ss-4)->currentMove.is_ok())
            {
                Square prevTo = (ss-4)->currentMove.to_sq();
                Piece prevPc = pos.piece_on(prevTo);
                ttMoveHistory[prevPc][prevTo][pos.moved_piece(ttData.move)][ttData.move.to_sq()] << (bonus * 32 / 137);
            }
            if ((ss-6)->currentMove.is_ok())
            {
                Square prevTo = (ss-6)->currentMove.to_sq();
                Piece prevPc = pos.piece_on(prevTo);
                ttMoveHistory[prevPc][prevTo][pos.moved_piece(ttData.move)][ttData.move.to_sq()] << (bonus * 16 / 137);
            }
            if ((ss-8)->currentMove.is_ok())
            {
                Square prevTo = (ss-8)->currentMove.to_sq();
                Piece prevPc = pos.piece_on(prevTo);
                ttMoveHistory[prevPc][prevTo][pos.moved_piece(ttData.move)][ttData.move.to_sq()] << (bonus * 8 / 137);
            }
        }
    }
